...
export default {
  data() {
    ...
    methods: {
      //求和
      getSum(){
        var arr=[this.normalBalance,this.followBalance,this.minorBalance,this.suspiciousBalance,this.lossBalance]      
        var arr1=[this.lossReal,this.suspiciousReal,this.minorReal,this.followReal,this.normalReal]
      ...
      overchange(val){
        this.searchform.dateMonth=val;
         this.$http
        .post(
            this.$store.state.domain +"/report/supAssetLoss/detail",this.searchform
        )
      ...
      gettime(){
         this.$http
        .post(
            this.$store.state.domain +"/report/supAssetLoss/getDateMonth"
        )
        //then()方法异步执行，就是当then()前面的方法执行完之后再执行then()里面的方法，这样就不会发生获取不到数据的问题
        .then(  
            // Lambda写法
